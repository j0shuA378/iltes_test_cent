      ...
      <div className="flex items-center gap-2.5">
        ...
        {activeUser.currentBand === 0 && onOpenPlacementTest && (
          <button
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/[0.04] hover:bg-black/[0.07] text-[#1d1d1f] text-xs font-medium border border-black/[0.06] transition-all cursor-pointer active:scale-98"
            title="完成 3 分钟学术定级测验"
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[#1d1d1f] animate-pulse"></span>
            <span>测定初始成绩</span>
          </button>
        )}
        ...
        >
          <Brain className="w-3.5 h-3.5 text-[#86868b]" />
          <span className="hidden md:inline text-[#86868b]">抗遗忘:</span>
          {dueVocabCount > 0 ? (
            <span className="font-semibold text-[#1d1d1f]">{dueVocabCount} 词待复核</span>
          ) : (
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/[0.03] text-[#86868b] text-xs font-normal border border-black/[0.04]">
          ...
          <span>距考期 <strong className="text-[#1d1d1f] font-semibold tabular-nums">{remainingDays}</strong> 天</span>
        </div>
        ...
        >
          ...
          <div className="text-left hidden sm:block leading-none">
            ...
            {activeUser.currentBand === 0 && (
              <div className="text-[10px] text-[#86868b] font-medium mt-0.5">待定级</div>
            )}
          </div>
        </button>
      </div>
